  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isAdminMode || isLogin) {
        ...
        // Direct signup, no email/OTP round-trip — see auth.service.ts's
        // signup for why (email delivery isn't reliably reachable in
        // production right now). AuthContext hydrates the session on
        // success; the router redirects away from here on its own.
      ...
      <div className="max-w-5xl w-full grid md:grid-cols-2 gap-12 lg:gap-24 items-center animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="bg-white p-8 lg:p-10 rounded-[2rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-gray-100 relative">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-400 font-medium">{isLogin ? 'Enter your details to continue.' : 'Create your account to get started.'}</p>
          </div>
